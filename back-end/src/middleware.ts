...
async function addHttpIntegration(applicationId: string): Promise<void> {
...
async function addDevice(applicationId: string, devEUI: string, deviceProfileId: string, deviceName: string): Promise<string> {
...
async function addDeviceKeys(devEUI: string, appKey: string): Promise<void> {
...
export const chirpstack = async function main() {
  try {
    const applicationId = await createApplication();
    await addHttpIntegration(applicationId);
    ...
    const deviceId = await addDevice(applicationId, devEUI, deviceProfileId, 'my-new-device');
    await addDeviceKeys(deviceId, appKey);

    console.log('Otomatisasi berhasil dilakukan.');
  } catch (error) {
