 ...
 */
async function testHTTPPayload(
  deviceId: string,
  secret: string
): Promise<boolean> {
  try {
    ...
    if (response.ok) {
      const result = await response.json();
      console.log(
        `✅ HTTP test successful for device ${deviceId}:`,
        result.message
      );
      return true;
    } else {
...
async function testHTTPPayloadWithAlarms(
  ...
  try {
    ...
    const encryptedPayload = encryptPayload(payload, secret);
    console.log(
      `🔒 Base64 encrypted payload: ${encryptedPayload.substring(0, 64)}...`
    );
    ...
    if (response.ok) {
      ...
      console.log(
        "🔄 Server Process: JWT Decode -> Base64 Decrypt -> Save Data -> Check Alarms → Send WhatsApp! 📱"
      );
    ...
    client.on("connect", () => {
      ...
      try {
        ...
        client.publish(MQTT_DEVICE.topic, mqttMessage, (error) => {
          if (error) {
            console.log(
              `❌ MQTT publish failed for device ${deviceId}:`,
              error.message
            );
            published = false;
          } else {
            console.log(
              `✅ MQTT published successfully for device ${deviceId}`
            );
            published = true;
          }
...
async function testMQTTPayloadWithAlarms(
  ...
  return new Promise((resolve) => {
    ...
    console.log(
      "🔄 Business Logic: Sensor Data -> Base64 Encrypt -> JWT -> MQTT Publish"
    );
    ...
    client.on("connect", () => {
      ...
      try {
        ...
        client.publish(MQTT_DEVICE.topic, mqttMessage, (error) => {
          if (error) {
            console.log(
              `❌ MQTT ALARM publish failed for device ${deviceId}:`,
              error.message
            );
            published = false;
          } else {
            console.log(
              `✅ MQTT ALARM published successfully for device ${deviceId}`
            );
            console.log(
              "🔄 Server Process: MQTT Receive -> JWT Decode -> Base64 Decrypt -> Save Data -> Check Alarms → Send WhatsApp! 📱"
            );
            console.log(`📱 Expected notifications to: 6283119720725`);
            console.log(`🚨 Alarm 1: pH = 8.5 > 8.0 threshold`);
            console.log(`🚨 Alarm 2: Flow = 12.0 < 15.0 threshold`);
...
async function runCompleteTest(): Promise<void> {
  console.log(
    "🧪 COMPLETE TESTER WITH ALARMS - All-in-One Testing & Debugging (NO AES)"
  );
  console.log(
    "=============================================================================="
  );
  ...
  console.log(
    "  Server: Receive -> Decode JWT -> Base64 Decrypt -> Save -> Check Alarms → WhatsApp!"
  );
  console.log("");
  ...
  results.httpNormalTest = await testHTTPPayload(
    "1",
  ...
  results.mqttNormalTest = await testMQTTPublish(
    "2",
  ...
  console.log("========================");
  console.log(
    `🔍 Secret Check: ${results.secretCheck ? "✅ PASS" : "❌ FAIL"}`
  );
  console.log(
    `🔧 Alarms Config: ${results.alarmsConfig ? "✅ PASS" : "❌ FAIL"}`
  );
  console.log(
    `📡 Alarm API Endpoints: ${
      results.alarmApiEndpoints ? "✅ PASS" : "❌ FAIL"
    }`
  );
  console.log(
    `📡 HTTP Normal Test: ${results.httpNormalTest ? "✅ PASS" : "❌ FAIL"}`
  );
  console.log(
    `🚨 HTTP Alarm Test: ${results.httpAlarmTest ? "✅ PASS" : "❌ FAIL"}`
  );
  console.log(
    `📡 MQTT Normal Test: ${results.mqttNormalTest ? "✅ PASS" : "❌ FAIL"}`
  );
  console.log(
    `🚨 MQTT Alarm Test: ${results.mqttAlarmTest ? "✅ PASS" : "❌ FAIL"}`
  );
  console.log(
    `📱 Alarm Notifications Check: ${
      results.alarmNotifications ? "✅ PASS" : "❌ FAIL"
    }`
  );
